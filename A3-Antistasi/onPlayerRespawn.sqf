...
if (side group player == teamPlayer) then
	...
	if (_owner != _oldUnit) exitWith {[localize "STR_antistasi_customHint_remoteAI", localize "STR_antistasi_customHint_remoteAI_dead"] call A3A_fnc_customHint; selectPlayer _owner; disableUserInput false; deleteVehicle _newUnit};
	...
	player addEventHandler ["WeaponAssembled",
		{
			...
			if (_veh isKindOf "StaticWeapon") then {
				...
				_pos = position _veh;
				if (_markersX findIf {_pos inArea _x} != -1) then {[localize "STR_antistasi_customHint_staticDeployed", localize "STR_antistasi_customHint_staticDeployed_text"] call A3A_fnc_customHint;};
			};
		}];
