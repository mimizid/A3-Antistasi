...
//if (!allowPlayerRecruit) exitWith {hint "Server is very loaded. <br/>Wait one minute or change FPS settings in order to fulfill this request"};
	if (!([player] call A3A_fnc_hasRadio)) exitWith {if !(hasIFA) then {[localize "STR_antistasi_customHint_air_support", localize "STR_antistasi_customHint_minefields_noradio"] call A3A_fnc_customHint;} else {[localize "STR_antistasi_customHint_air_support", localize "STR_antistasi_customHint_minefields_no_radioman"] call A3A_fnc_customHint;}};
